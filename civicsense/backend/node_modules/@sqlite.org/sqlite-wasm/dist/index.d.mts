...
type Worker1OptionalArgsOp = { [K in keyof Worker1ArgsMap]: undefined extends Worker1ArgsMap[K] ? K : never }[keyof Worker1ArgsMap];
type Worker1RequiredArgsOp = Exclude<keyof Worker1ArgsMap, Worker1OptionalArgsOp>;
...
type Worker1Promiser = {
   ...
   * response message.
   */
  <T extends Worker1RequiredArgsOp>(type: T, args: Worker1ArgsMap[T]): Promise<Worker1OutputEnvelope<T, Worker1ResultMap[T]>>;
  /**
   * Sends a message to the worker and returns a Promise which resolves to the
   * response message.
   */
  <T extends Worker1OptionalArgsOp>(type: T, args?: Worker1ArgsMap[T]): Promise<Worker1OutputEnvelope<T, Worker1ResultMap[T]>>;
  /**
   * Sends a message to the worker and returns a Promise which resolves to the
   * response message.
   */
